...
class App extends React.Component<AppProps, AppState> {
                    ...
                    >
                      ...
                      >
                        <LayerUI
                          ...
                          openAIBaseURL={this.OPENAI_BASE_URL}
                          openAIModelName={this.OPENAI_MODEL_NAME}
                        >
                          {this.props.children}
                        </LayerUI>
  ...
  private async onMagicFrameGenerate(
    ...
    const result = await diagramToHTML({
      ...
      modelName: this.OPENAI_MODEL_NAME,
    });
  ...
  private OPENAI_KEY: string | null =
    EditorLocalStorage.get(EDITOR_LS_KEYS.OAI_API_KEY) || "123";
  private OPENAI_BASE_URL: string | null =
    EditorLocalStorage.get(EDITOR_LS_KEYS.OAI_BASE_URL) || "/api/v2";
  private OPENAI_MODEL_NAME: string | null =
    EditorLocalStorage.get(EDITOR_LS_KEYS.OAI_MODEL_NAME) || "gpt-4.1-mini";

  private OPENAI_KEY_IS_PERSISTED: boolean =
    EditorLocalStorage.has(EDITOR_LS_KEYS.OAI_API_KEY_IS_PERSISTED) || false;

  private onOpenAIKeyChange = (settings: {
    key: string | null;
    baseURL: string | null;
    modelName: string | null;
    shouldPersist: boolean;
  }) => {
    const { key, baseURL, modelName, shouldPersist } = settings;
    this.OPENAI_KEY = key;
    this.OPENAI_BASE_URL = baseURL;
    this.OPENAI_MODEL_NAME = modelName;
    this.OPENAI_KEY_IS_PERSISTED = shouldPersist;
    if (shouldPersist) {
      EditorLocalStorage.set(EDITOR_LS_KEYS.OAI_API_KEY, key);
      EditorLocalStorage.set(EDITOR_LS_KEYS.OAI_BASE_URL, baseURL);
      EditorLocalStorage.set(EDITOR_LS_KEYS.OAI_MODEL_NAME, modelName);
    } else {
      EditorLocalStorage.delete(EDITOR_LS_KEYS.OAI_API_KEY);
      EditorLocalStorage.delete(EDITOR_LS_KEYS.OAI_BASE_URL);
      EditorLocalStorage.delete(EDITOR_LS_KEYS.OAI_MODEL_NAME);
    }
  };
  ...
  private onMagicSettingsConfirm = (
    ...
    this.onOpenAIKeyChange(settings);
    EditorLocalStorage.set(
      EDITOR_LS_KEYS.OAI_API_KEY_IS_PERSISTED,
      settings.shouldPersist,
    );
